...
const Course = require('./CourseSchema');   // used in populate
const majorsRoutes = require('./RouteMajors'); // make sure filename matches
...
const mongoString =
  ...
  "mongodb+srv://ggduplan23:IbfP35dg8N2etsBm@cluster0.ycfq01g.mongodb.net/cluster0";
...
app.post('/createUser', async (req, res) => {
  console.log(
    `SERVER: CREATE USER REQ BODY: ${req.body.username} ${req.body.firstName} ${req.body.lastName}`
  );
...
app.post('/createPlan', async (req, res) => {
  try {
    ...
    const plan = new Plan({
      ...
      desiredGradTerm: desiredGradTerm || null,
      desiredGradYear: desiredGradYear || null,
    });

    const savedPlan = await plan.save();

    // Add plan to user's plans array (NOT schedules)
...
app.put('/updatePlanVisibility', async (req, res) => {
  try {
    ...
    if (!planId || isPublic === undefined) {
      return res
        .status(400)
        .send({ error: 'PlanId and public status are required' });
    }
    ...
    if (userId && plan.owner.toString() !== userId) {
      return res
        .status(403)
        .send({ error: 'You do not have permission to modify this plan' });
    }
